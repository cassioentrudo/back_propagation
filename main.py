...
def initialize_network_for_validation(network_file_lines, initial_weights_file_lines, dataset_file_lines, isTest, network = None):
    ...
    if (network == None):
        ...
        errorReg, network = back_propagation.execute(neural_network, dataset_file_lines, isTest)
        
    else:
        errorReg, network = back_propagation.execute(network, dataset_file_lines, isTest)
...
def main():
    ...
    if(len(attributes_command_line) > 1):
        ...
        for i in range(len(folds)):
            ...
            errorReg,network = initialize_network_for_validation(neural_network_structure, empty_initial_weights, fixed_dataset, False)
            while (errorReg>0.1):
                network.a=[]
                network.z=[]
                errorReg,network = initialize_network_for_validation(neural_network_structure, empty_initial_weights, fixed_dataset, False, network)
